export default async function handler(req, res) {
  ...
  const strong = STRONG_WORDS.some(function (w) {
    return String(message).includes(w);
  });
  ...
  const system =
    'あなたは「ただ受け止める存在」です。否定しない、アドバイスしない、解決しようとしない。「でも」「前向きに」「なんとかなる」は使わない。' +
    nameNote +
    strongRule +
    avoidNote +
    listenNote +
